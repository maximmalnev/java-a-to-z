    ...
    $("#logout-button").click(function () {
        $.ajax({
            ...
            success: function () {
                ...
                clearEditForm();
                if (currentClient.role == "user") {
                    showBlock($("#creation-block-button"));
                    showBlock($("#edition-block-button"));
                    showBlock($("#delete-block-button"));
                    $("#edit-role-id").removeAttr('disabled');
                }
                hideBlock($("#table-block"));
                hideBlock($("#user-buttons-block"));
                $("#auth-block").find("input").val('');
                $("thead").empty();
                tableBody.empty();
                $(".welcome").text('guest');
                currentClient = null;
                showBlock($("#auth-block"), animationTime);
            }
        });
    });
