...
/**
 * Create user and return all users info.
 */
public class CreateUser extends HttpServlet {

    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        Optional<User> user = Validator.getInstance().getUserForCreation(req);
        if (user.isPresent()) {
            Writer writer = resp.getWriter();
            List<User> users = PostgreManager.getInstance().add(user.get());
